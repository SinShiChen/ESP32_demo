...
#define TEST_BOARD_I2S_DATA_IN_PIN  (GPIO_NUM_12)
...
void app_main(void)
{
    ...
    i2c_cfg.addr = ES7210_CODEC_DEFAULT_ADDR+2;
    ...
    TEST_ESP_OK(ret);
    ret = esp_codec_dev_set_in_gain(record_dev, 60.0);
    TEST_ESP_OK(ret);
    ...
    const wav_header_t wav_header =
        WAV_HEADER_PCM_DEFAULT(limit_size, fs.bits_per_sample, fs.sample_rate, fs.channel);
    ESP_LOGI(TAG, "Opening file %s", EXAMPLE_RECORD_FILE_PATH);
    FILE *f = fopen(EXAMPLE_SD_MOUNT_POINT EXAMPLE_RECORD_FILE_PATH, "w");
    if (f == NULL)
    {
        ESP_LOGI(TAG, "Opening file %s fail", EXAMPLE_RECORD_FILE_PATH);
        while (1)
            vTaskDelay(100);
    }
    ...
    ESP_LOGI("e","break");
    esp_err_t err;
    err = esp_vfs_fat_sdcard_unmount(EXAMPLE_SD_MOUNT_POINT, sdmmc_card);
    if (err == ESP_OK) {
