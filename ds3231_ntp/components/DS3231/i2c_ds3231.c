...
esp_err_t ds3231_set_time(i2c_master_dev_handle_t *dev, struct tm *time)
{
	...
	ESP_LOGI("ds3231","ime->tm_year %d ",time->tm_year);
...
esp_err_t ds3231_get_time(i2c_master_dev_handle_t *dev, struct tm *time)
{
	...
	ESP_LOGI("DS3231", "tm_year %d tm_mon %d  tm_wday %d tm_hour %d",time->tm_year,time->tm_mon,time->tm_wday,time->tm_hour);
